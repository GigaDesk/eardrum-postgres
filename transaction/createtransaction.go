...
func ProcessOrder(db *gorm.DB, shopID uint, newTx transaction.NewProductsTransaction, checkPIN func(hashedPIN, PIN string) error) (*Transaction, error) {
	...
	err := db.Transaction(func(tx *gorm.DB) error {
		...
		for _, item := range newTx.GetPurchasedProducts() {
			...
			if p.Blocked || p.Deleted {
				return fmt.Errorf("product with ID %d is not available for purchase", item.GetProductID())
			}
...
// necessary database records securely within a single transaction.
// checkPIN is a function passed as an argument to decouple logic.
func ProcessTransaction(db *gorm.DB, shopID uint, newTx transaction.NewTransaction, checkPIN func(hashedPIN, PIN string) error) (*Transaction, error) {
	// We use GORM's built-in transaction helper to ensure all operations are
	// either fully completed or fully rolled back if an error occurs.
	var newTransaction *Transaction
	err := db.Transaction(func(tx *gorm.DB) error {
		...
		// =========================================================================
		// 3. ACCOUNT UPDATES (LOCKING BOTH USER AND SHOP)
		// =========================================================================
		// Find the shop and lock its row within this same transaction to prevent race conditions.
		var s shop.Shop
		if err := tx.Clauses(clause.Locking{Strength: "UPDATE"}).
